...
public class GetAccessToken {
    ...
    // 设置 AccessToken 授权应用可见范围
    @ParameterizedTest
    @MethodSource("com.vernon.qywechat.QYWeChatSource#setScopeTest")
    public DocumentContext setScope(SetScopeDTO setScopeDTO) {
        String scopeURI = "/cgi-bin/agent/set_scope";
        HashMap<String,Object> params = new HashMap<>(){{
            put("agentId",setScopeDTO.getAgentId());
            put("allowUser",setScopeDTO.getAllowUser());
            put("allowParty",setScopeDTO.getAllowParty());
            put("allowTag",setScopeDTO.getAllowTag());
        }};
        Response response = given()
                .log().all()
